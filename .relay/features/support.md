...
The app never claims a staffed queue, human response, SLA, or external ticket unless one exists. The public site exposes `mikey641@gmail.com` as the support and privacy contact through a normal `mailto:` link, but no response SLA or ticket queue is claimed. A GitHub integration is enabled only when `ENSYNC_GITHUB_ISSUES_URL` is an exact HTTPS `github.com/<owner>/<repo>/issues/new` URL; even then Ensync prepares an unsent reviewed draft and never submits it.
