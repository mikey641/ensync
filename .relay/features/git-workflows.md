...
If an item conflicts, the integrator may invoke an authenticated subscription runner (Codex, Claude Code, or Factory Droid) with a bounded conflict-only prompt and an OS-enforced boundary around the integration worktree. The originating provider is tried first, then the remaining supported resolvers; Cursor remains discovery-only until it can prove paid overage is disabled. Canonical realpaths and the shared Git directory are verified before it starts; the provider process must terminate, conflict markers must be gone, and files outside the reported conflict set must remain identical before the resolution is accepted. An unresolved item returns to durable retry state and is automatically retrained at startup or ahead of the next completion for that repository; later compatible items are still allowed to land. After the conflict-fallback threshold is reached (default 3 attempts), the integrator automatically resolves remaining conflict files by checking out the incoming (theirs) version, staging, and completing the merge with the same post-resolution verification (no markers, saved-commit ancestry, protected-entries unchanged). This ensures stuck items self-resolve without manual intervention. There is no automatic-landing toggle, environment opt-out, polling interval, filesystem repository lease, force merge, or manual `Needs merge review` state.
