...
Lease heartbeat ownership metadata is replaced atomically through a same-directory staging file, and one Host never overlaps its own heartbeat writes. Observers therefore see either the complete previous record or the complete next record; a transient in-place truncation must never be misclassified as lost ownership. Token mismatch, missing ownership, and genuine write failure still fail closed before further provider work.
