...
All local explicit and automatic land operations serialize through one renewable repository-scoped lease in Git's common directory. After entering that queue, the land operation freshly rechecks the checkout, branch, overlaps, merge conflicts, and repository land gate. A stale lease owner is identified by PID plus process start time, with only a bounded grace for timestamp precision; because operating systems reuse PIDs, PID liveness alone must never fence the landing queue indefinitely. File overlap stays advisory; dirty shared state, Git conflicts, and semantic verification failures continue to fail closed. Automatic landing is on by default and can be disabled host-wide with `ENSYNC_AUTO_LAND=0` (or the `autoLandAgentWork` host option). It is also a user preference: the renderer's Automatic landing settings toggle (on by default, persisted with the workspace snapshot) travels with each local run request as its optional boolean `autoLand` field, and `autoLand: false` keeps that run's branch unlanded for explicit review. Because the shared Host daemon outlives any one window, the preference is carried per request rather than stored host-side; a request can opt out of landing but never re-enable it when the host-wide switch has it off.
