...
- A signed-in account is server-first: one always-on Ensync Server owns projects, subscription CLI logins, provider sessions, protected worktrees, and jobs, while every computer is a live authenticated client of that same authority. Username identifies the account; password/session authentication grants access. A VM is optional deployment isolation, not a client-sync requirement.
