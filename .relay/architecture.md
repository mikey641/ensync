...
7. Ensync Sync is the account and remote-control plane for encrypted conversation documents and brokered agent execution. Authenticated iOS, Android, web, and desktop clients send end-to-end encrypted job requests through Sync to an explicitly paired Ensync Host or worker. From the client's perspective this is remote execution through Sync; operationally, that selected Host remains the execution target and owns project verification, subscription CLI credentials and sessions, protected worktrees, job journaling, approvals, execution, steering, and cancellation. Sync owns device identity, pairing and two-sided revocation, encrypted routing, delivery receipts, and minimal wake/notification metadata. It may retain opaque encrypted application payloads plus the minimum routing and idempotency metadata, but never runs a provider CLI or receives plaintext prompts, source files, provider credentials, or decryption keys.
