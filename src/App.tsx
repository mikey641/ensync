...
import { DisplayPreferences, useDisplayPreferences, type DisplayPreferencesState } from './display-preferences'
import {
...
const EMPTY_ACCOUNT_SYNC_STATUS: AccountSyncStatus = {
  ...
  executionAuthority: 'device_host',
}
...
type StoredState = {
  ...
  /** Account-portable appearance preferences; physical device choices remain separate. */
  displayPreferences?: DisplayPreferencesState
  /** Account/server-wide provider maintenance policy and cadence. */
  agentUpdatePreferences?: AgentUpdatePreferences
...
function App() {
  ...
  const accountSyncSettingsFingerprintRef = useRef<string | null>(null)
  const accountSyncRevisionRef = useRef(0)
  const displayPreferencesRef = useRef<DisplayPreferencesState>({ theme, textSize, completionIndicator })
  const agentUpdatePreferencesRef = useRef(agentUpdatePreferences)
  ...
  displayPreferencesRef.current = { theme, textSize, completionIndicator }
  agentUpdatePreferencesRef.current = agentUpdatePreferences
  accountSyncRevisionRef.current = accountSyncStatus.remoteRevision ?? 0
  ...
  const synchronizeAccountWorkspace = useCallback(() => {
    ...
    const run = (async () => {
      ...
      try {
        ...
        const localSource = {
          ...
          displayPreferences: displayPreferencesRef.current,
          agentUpdatePreferences: agentUpdatePreferencesRef.current,
        }
        const localPortable = prepareAccountWorkspace(localSource)
        const localSettingsFingerprint = JSON.stringify(localPortable.settings)
        const localSettingsDirty = accountSyncSettingsFingerprintRef.current !== null
          && localSettingsFingerprint !== accountSyncSettingsFingerprintRef.current
        let merged = mergeAccountWorkspace(localSource, pulled.state, {
          preferLocalSettings: !accountWorkspaceHasSettings(pulled.state) || localSettingsDirty,
        })
        ...
        if (!remotePortable || JSON.stringify(remotePortable) !== JSON.stringify(portable)) {
          ...
          if (pushed.status === 'conflict') {
            merged = mergeAccountWorkspace(merged.state, pushed.remoteState, {
              preferLocalSettings: !accountWorkspaceHasSettings(pushed.remoteState) || localSettingsDirty,
            })
        ...
        const nextChats = (nextState.chats ?? []).map(normalizeChatModelChoice)
        const nextSessions = nextState.chatSessions ?? {}
        const nextInFlightRuns = nextState.inFlightRuns ?? {}
        const nextExecutionEvents = nextState.chatExecutionEvents ?? {}
        setChats(nextChats)
        ...
        setChatExecutionEvents(nextExecutionEvents)
        chatExecutionEventsRef.current = nextExecutionEvents
        ...
        const nextPlacement = nextState.placement === 'end' ? 'end' : 'adjacent'
        const nextConversationLayout = nextState.conversationLayout === 'tabs' ? 'tabs' : 'split'
        const nextAutoFallback = nextState.autoFallback !== false
        const nextAutoContextSkill = nextState.autoContextSkill === true
        const nextFallbackProviderOrder = normalizeFallbackProviderOrder(nextState.fallbackProviderOrder)
        const nextDisplayPreferences = nextState.displayPreferences ?? displayPreferencesRef.current
        const nextAgentUpdatePreferences = writeAgentUpdatePreferences(
          window.localStorage,
          nextState.agentUpdatePreferences ?? agentUpdatePreferencesRef.current,
        )
        setPlacement(nextPlacement)
        setConversationLayout(nextConversationLayout)
        setAutoFallback(nextAutoFallback)
        setAutoContextSkill(nextAutoContextSkill)
        setFallbackProviderOrder(nextFallbackProviderOrder)
        setTheme(nextDisplayPreferences.theme)
        setTextSize(nextDisplayPreferences.textSize)
        setCompletionIndicator(nextDisplayPreferences.completionIndicator)
        displayPreferencesRef.current = nextDisplayPreferences
        setAgentUpdatePreferences(nextAgentUpdatePreferences)
        agentUpdatePreferencesRef.current = nextAgentUpdatePreferences
        commitWorkspace({
          chats: nextChats,
          projects: nextProjects,
          chatSessions: nextSessions,
          inFlightRuns: nextInFlightRuns,
          chatExecutionEvents: nextExecutionEvents,
          placement: nextPlacement,
          conversationLayout: nextConversationLayout,
          autoFallback: nextAutoFallback,
          autoContextSkill: nextAutoContextSkill,
          fallbackProviderOrder: nextFallbackProviderOrder,
        })
        accountSyncFingerprintRef.current = JSON.stringify(portable)
        accountSyncSettingsFingerprintRef.current = JSON.stringify(portable.settings)
        ...
        setAccountSyncMessage(
          merged.importedChats > 0
            ...
            : updatedAt
              ? 'Conversations and settings are up to date.'
              : 'Your conversations and settings are now protected by account sync.',
        )
      } catch (error) {
    ...
    return run
  }, [commitWorkspace, setCompletionIndicator, setTextSize, setTheme])
  ...
  const logoutAccountSync = useCallback(async () => {
    ...
    try {
      ...
      accountSyncSettingsFingerprintRef.current = null
      setAccountSyncPhase('idle')
    } catch (error) {
      setAccountSyncPhase('error')
  ...
  const accountWorkspaceDocument = useMemo(
    () => prepareAccountWorkspace({
      ...
      autoContextSkill,
      fallbackProviderOrder,
      displayPreferences: { theme, textSize, completionIndicator },
      agentUpdatePreferences,
    }),
    [agentUpdatePreferences, autoContextSkill, autoFallback, chatExecutionEvents, chatSessions, chats, completionIndicator, conversationLayout, fallbackProviderOrder, inFlightRuns, placement, projects, textSize, theme],
  )
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      void accountSyncHost.subscribe((event) => {
        retryDelay = 500
        if (event.type !== 'workspace_updated'
          || event.revision <= accountSyncRevisionRef.current) return
        void synchronizeAccountWorkspace().catch(() => {})
      }, {
        signal: controller.signal,
        afterRevision: accountSyncRevisionRef.current,
      }).catch((error: unknown) => {
        if (stopped || controller?.signal.aborted) return
        setAccountSyncMessage(error instanceof Error ? error.message : 'The live account connection was interrupted.')
      }).finally(() => {
        if (stopped) return
  ...
  }, [inFlightRuns, recoverDetachedRun])
  ...
  return (
    <section className="setting-section account-sync-setting">
      <div className="setting-title">
        <div><h3>Live server account</h3><p>Sign in once and receive server-owned conversations and account settings on every connected computer immediately.</p></div>
        {status.authenticated && <span className="account-sync-badge"><i /> LIVE</span>}
      </div>
      {phase === 'checking' ? (
        ...
        <div className="account-sync-connected">
          <div className="account-sync-profile"><span><UserRound size={18} /></span><p><strong>{status.username}</strong><small>{chatCount} {chatCount === 1 ? 'conversation' : 'conversations'} · live revision {status.remoteRevision ?? 0} · updated {lastSynced}</small></p><span className="account-sync-encryption"><LockKeyhole size={13} /> AES-256</span></div>
          <div className="account-sync-actions">
            <p><ShieldCheck size={14} /><span>The signed-in Ensync Host owns execution. Conversations and portable settings are encrypted and pushed live to every client; provider credentials and physical-device preferences never enter the account document.</span></p>
            <button type="button" className="button button--ghost" onClick={() => void onSync().catch(() => {})} disabled={busy}><RotateCw className={busy ? 'spin' : ''} size={14} /> {busy ? 'Refreshing…' : 'Refresh now'}</button>
            <button type="button" className="button button--ghost" onClick={() => void onLogout()} disabled={busy}><LogOut size={14} /> Sign out</button>
          </div>
          <small className="account-sync-session-note">The live stream reconnects automatically and catches up from the last server revision. Login remains in Ensync Host memory, so restarting that Host requires signing in again.</small>
        </div>
      ) : (
