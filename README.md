...
- Username/password live account sync for server-owned conversations, reconnectable jobs, and portable settings. Theme, text size, conversation layout, fallback/Auto Context policy, and server agent-update policy follow the account; credentials, drafts, window geometry, completion audio/voice, and local attachment paths do not.
...
For live use from multiple computers, run `npm run sync-service` on one always-on server and put it behind HTTPS. This starts both the account service and a private loopback Ensync execution Host. Configure its persistent paths with `ENSYNC_SYNC_DATA_FILE`, `ENSYNC_SERVER_JOB_JOURNAL_FILE`, and `ENSYNC_SERVER_WORKSPACE_ROOT`; set `ENSYNC_SERVER_PROJECT_PATH` when the server should open one project by default. Install and subscription-authenticate supported provider CLIs on that server, then set the same HTTPS `ENSYNC_SYNC_SERVICE_URL` on every Ensync desktop client.

After username/password login, provider discovery, project inspection, Git operations, chat jobs, retained streams, steering, and cancellation run on the account server. Conversation and bounded execution-state revisions are AES-256-GCM encrypted and announced through an authenticated live event stream, so another signed-in computer pulls changes within seconds and can attach to the same Host job. Account and encryption credentials stay in each client Host's memory; provider credentials and source worktrees stay on the server. Plain HTTP is accepted only for an exact loopback development service. A public deployment must use HTTPS and, before serving unrelated users, a separate operating-system or VM isolation boundary per account.
