...
Codex and Claude Code are the enabled structured subscription runners. Cursor and Droid have implemented adapters but stay discovery-only because account login and quota telemetry do not prove paid Additional/Extra Usage is disabled. Ollama remains a separate local runtime and never enters the subscription pool.
