...
- Provider handoff preserves the shared task identity, relevant transcript, canonical project, `.ensync` feature memory and plan, root `CLAUDE.md` and `AGENTS.md` adapters, and host-verified Git branch/status. Vendor session IDs remain provider-specific below that shared task identity.
...
- Successful local coding runs preserve an exact commit and finish immediately after applying the prompt's captured delivery destination. Production remains the default and enqueues automatic landing; **Protected branch only** durably anchors the exact commit without merging, pushing, or deploying it. There is no manual merge-review state: production deliveries integrate in completion-order FIFO trains, batch arrivals that finish while a train is active, and let a conflicted item retry without blocking compatible saved work.
