...
function accountLiveHeaders(origin) {
  return {
    ...responseHeaders(origin),
...
function sendAccountLiveEvent(response, event) {
  if (response.destroyed || response.writableEnded) return
...
function readRawBody(request, maxBytes = MAX_SYNC_BODY_BYTES) {
  return new Promise((resolve, reject) => {
    const chunks = []
    let bytes = 0
    request.on('data', (chunk) => {
      bytes += chunk.length
      if (bytes > maxBytes) {
        reject(new AccountSyncError('server_request_too_large', 'The Ensync server request is too large.', 413))
        request.destroy()
        return
      }
      chunks.push(chunk)
...
async function proxyAccountServerRequest({ accountSync, request, response, url, origin }) {
  ...
  try {
    ...
    const upstream = await accountSync.executionRequest(`${url.pathname}${url.search}`, {
      method,
...
export function createEnsyncHost(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/account-sync/events') {
        const cancellation = requestCancellation(request, response)
        let opened = false
        try {
          await accountSync.subscribe({
            signal: cancellation.signal,
            afterRevision: Number.parseInt(url.searchParams.get('after') ?? '0', 10),
            onOpen: () => {
              opened = true
              response.writeHead(200, accountLiveHeaders(origin))
            },
            onEvent: (event) => sendAccountLiveEvent(response, event),
          })
          if (!response.destroyed && !response.writableEnded) response.end()
          return
        } catch (error) {
      ...
      if (accountSync.usesAccountServerExecution?.()) {
        return await proxyAccountServerRequest({ accountSync, request, response, url, origin })
      }
