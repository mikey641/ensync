...
import { chmod, mkdir, mkdtemp, readFile, realpath, rename, rm, stat, utimes, writeFile } from 'node:fs/promises'
    ...
    for (;;) {
      ...
      try {
        ...
        const writeOwner = async () => {
          const stagingPath = join(lockPath, `owner-${digest(`${token}:${this.#uuid()}`)}.tmp`)
          try {
            await writeFile(stagingPath, JSON.stringify(owner()), {
              encoding: 'utf8',
              mode: 0o600,
              flag: 'wx',
            })
            try { await chmod(stagingPath, 0o600) } catch { /* Windows ACLs remain user-scoped. */ }
            // Same-directory replacement keeps every observer on either the
            // complete prior heartbeat or the complete next heartbeat. Never
            // truncate owner.json in place: concurrent Host timers and release
            // checks must not mistake a partially written file for lease loss.
            await rename(stagingPath, ownerPath)
          } finally {
