...
test('the conversation pane offers exact owning-chat navigation without transferring a prompt', async () => {
  ...
  const bannerStart = app.indexOf('function OwningConversationBanner')
  const bannerEnd = app.indexOf('function ExecutionPanel', bannerStart)
  assert.notEqual(bannerStart, -1)
  assert.ok(bannerEnd > bannerStart)
  assert.match(app.slice(bannerStart, bannerEnd), /finally\s*{\s*setOpening\(false\)/)
