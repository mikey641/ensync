...
test('a renewing write lease is never lost because its own owner record was read mid-write', async (context) => {
  const fixture = await repositoryFixture(context)
  const commonDirectory = await git(fixture.repository, ['rev-parse', '--git-common-dir'])
  const key = 'window-a:chat-a'
  const ownerPath = join(workspaceLockPath(fixture.repository, commonDirectory, key), 'owner.json')
  const isolation = new ProjectIsolationService({ rootPath: fixture.workspaceRoot, heartbeatMs: 1 })
  const lease = await isolation.acquire(fixture.repository, key)

  try {
    for (let attempt = 0; attempt < 500; attempt += 1) {
      const record = JSON.parse(await readFile(ownerPath, 'utf8'))
      assert.equal(record.workspaceHash, createHash('sha256').update(key).digest('hex').slice(0, 24))
      lease.assertHeld()
    }
  } finally {
    await lease.release()
