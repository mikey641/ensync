...
test('the local hard run ceiling honors ENSYNC_CHAT_HARD_TIMEOUT_MS', async (context) => {
  const projectPath = await projectFixture(context)
  let capturedOptions
  const service = new ChatRunService({
    statusService: statusService(readyProvider('codex')),
  ...
  assert.equal(capturedOptions.hardTimeoutMs, 8 * 60 * 60 * 1_000)
  assert.equal(capturedOptions.inactivityTimeoutMs, 15 * 60 * 1_000)
})

test('an invalid local hard run ceiling falls back to 24 hours', async (context) => {
  const projectPath = await projectFixture(context)
  let capturedOptions
  const service = new ChatRunService({
    statusService: statusService(readyProvider('codex')),
    environment: { ENSYNC_CHAT_HARD_TIMEOUT_MS: 'unlimited' },
    processRunner: async (_executable, _args, options) => {
      capturedOptions = options
      return {
        exitCode: 0,
        ...
        stderr: '',
        stdout: [
          JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'Fallback ceiling response' } }),
          JSON.stringify({ type: 'turn.completed' }),
        ].join('\n'),
      }
    },
  ...
  await service.run({ provider: 'codex', projectPath, prompt: 'Check the fallback ceiling' })
