...
test('every mounted protected chat owns a collapsed branch-scoped production disclosure', async () => {
  ...
  assert.match(app, /<strong>\{hasUnsavedActivePrompt \? 'Current prompt delivery' : 'Production delivery'\}<\/strong>/)
  assert.match(app, /<em>This chat · \{label\}<\/em>/)
  assert.match(app, /hasUnsavedActivePrompt \? 'Running' : deliveryLabelText/)
  assert.match(app, /hasUnsavedActivePrompt\s*\? 'Current prompt · not saved yet'/)
  assert.match(app, /<strong>This chat only<\/strong>[\s\S]*?<code title=\{sourceBranch\}>\{sourceBranch\}<\/code>/)
  assert.match(app, /Open \$\{deliveryScope\?\.toLowerCase\(\) \?\? 'this prompt'\}’s verified deployment/)
