...
test('every mounted protected chat owns a collapsed branch-scoped production disclosure', async () => {
  ...
  assert.match(app, /<strong>Chat workspace<\/strong>/)
  assert.match(app, /Isolated source branch · Production status below/)
  ...
  assert.match(app, /Open \$\{deliverySubject\.toLowerCase\(\)\}’s/)
  ...
  assert.match(app, /const repairStepLabel = delivery\?\.state === 'repairing'/)
  assert.match(app, /\? 'Repairing'/)
  assert.match(app, /\? 'Repaired'/)
  assert.match(app, /: 'Repair if needed'/)
  assert.match(app, /const deliverySteps = \[[\s\S]*?'Working', 'Saved', landingStepLabel, 'Merge verified', 'Pushed', 'Building', repairStepLabel, 'Production',\s*\]/)
  assert.match(app, /const complete = promptIsActive\s*\?[\s\S]*?index === 0[\s\S]*?: delivery !== null && index <= deliveryProgressPosition\(delivery\) \+ 1/)
  assert.match(app, /earlier saved commits already merged/)
  assert.match(app, /Queue item.*is currently merging/)
  assert.match(app, /This exact saved commit is merged/)
  assert.match(app, /later.*commits remain.*repository queue after it/)
  assert.match(app, /commits remain.*ahead of this exact saved commit/)
  ...
  assert.doesNotMatch(app, /executionEvents=\{executionEvents\}/)
  assert.match(app, /Host-verified chain:/)
  assert.match(app, /Git ancestry was verified by Ensync Host/)
  assert.match(app, /const deliveryHasExactDeploymentLink = Boolean\(/)
  assert.match(app, /!delivery\.replacementCommitSha \|\| !delivery\.failureCode/)
  assert.match(app, /verified deployment · \$\{exactCommit\.slice\(0, 12\)\}/)
  assert.match(app, /Verified production'[\s\S]*?exactCommit/)
  assert.match(app, /prompts were handled in this one run/)
  ...
  assert.match(css, /\.delivery-panel__queue-progress\s*\{[^}]*height:\s*4px;/s)
