// Duplicated from host/process-liveness.mjs, deliberately.
//
// electron-builder packs only the files desktop/package.json lists under
// build.files into app.asar, and host/ is rsynced beside it as a separate
// tree. An import reaching from inside the asar into Resources/host/ resolves
// in a dev checkout and breaks only once installed — the exact failure this
// module exists to prevent. host/process-liveness.test.mjs asserts the two
// copies behave identically, so they cannot drift in silence.
