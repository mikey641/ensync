...
test('native bridge registers authenticated active-run publication and target-first handoff before windows load', async () => {
  ...
  assert.ok(bridgeBlock.includes('ACTIVE_RUN_CLAIM_CHANNEL'))
  assert.ok(bridgeBlock.includes('ACTIVE_RUN_CLAIM_FINALIZE_CHANNEL'))
  assert.ok(bridgeBlock.includes('ACTIVE_RUN_CLAIM_RELEASE_CHANNEL'))
...
test('preload exposes only fixed active-run and queued-message handoff bridge methods', async () => {
  ...
  assert.ok(source.includes("const ACTIVE_RUN_CLAIM_CHANNEL = 'ensync:workspace:claim-active-run'"))
  assert.ok(source.includes("const ACTIVE_RUN_CLAIM_FINALIZE_CHANNEL = 'ensync:workspace:finalize-active-run-claim'"))
  assert.ok(source.includes("const ACTIVE_RUN_CLAIM_RELEASE_CHANNEL = 'ensync:workspace:release-active-run-claim'"))
  ...
  assert.ok(source.includes('claimActiveRun: (request) => ipcRenderer.invoke(ACTIVE_RUN_CLAIM_CHANNEL, request)'))
  assert.ok(source.includes('finalizeActiveRunClaim: (request) => ipcRenderer.invoke(ACTIVE_RUN_CLAIM_FINALIZE_CHANNEL, request)'))
  assert.ok(source.includes('releaseActiveRunClaim: (request) => ipcRenderer.invoke(ACTIVE_RUN_CLAIM_RELEASE_CHANNEL, request)'))
...
test('renderer bridge types distinguish project, exact idle-chat, and exact active-run focus', async () => {
  ...
  assert.ok(source.includes("claimActiveRun?: (request: NativeExactRunClaimRequest) => Promise<{ status: 'claimed'; token: string } | { status: 'rejected' }>"))
  assert.ok(source.includes('finalizeActiveRunClaim?: (request: NativeExactRunClaimTokenRequest) => Promise<boolean>'))
  assert.ok(source.includes('releaseActiveRunClaim?: (request: NativeExactRunClaimTokenRequest) => Promise<boolean>'))
