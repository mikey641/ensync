...
`src/preload.cjs`, `src/project-picker.mjs`, and `src/update/*.mjs` are explicit package inputs. Keep the preload bridge narrow: a renderer may request only the fixed directory dialog and fixed update actions. The main process authorizes every request against the sending registered Ensync `BrowserWindow` and stable app URL. Folder cancellation changes no project state, and update actions share one truthful app-wide state across all native windows.

## Native updates

The updater is a port of VS Code's update service (`src/vs/platform/update/`), adapted in `src/update/`: `update.mjs` is upstream's `common/update.ts` state machine, `abstract-update-service.mjs` is `abstractUpdateService.ts`, and `update-service.{darwin,win32,linux}.mjs` are the per-platform services. States are upstream's — `Uninitialized → Idle → CheckingForUpdates → AvailableForDownload → Downloading → Downloaded → Updating → Ready` — and so is the scheduling: a first check 30 seconds after the first window opens, then hourly, honouring `update.mode` (`default`, `start`, `manual`, `none`) and the selected channel, both stored per device and applied without a restart.

Two things differ from upstream on purpose. Upstream delegates to Squirrel.Mac and Inno Setup, which need a live update server; Ensync publishes a static signed manifest and does the check, download, checksum and signature verification itself (`release-feed.mjs`, `installer-download.mjs`, `code-signature.mjs`). And `quitAndInstall()` is inert: `Ready` means "the verified installer is open", never "the app is about to restart into it".

Settings shows `app.getVersion()`, the embedded build ID, full source revision, clean/dirty flag, build time, packaging channel, selected update channel and mode, and the exact update state. Stable uses `https://ensync.vercel.app/releases.json`; opt-in beta uses `https://ensync.vercel.app/releases-beta.json`. It offers a release only when the current packaged app is signed and the matching-channel manifest has a newer matching platform version, a real HTTPS installer, a valid SHA-256, verified signing, and explicit macOS notarization. Stable rejects prerelease versions, and changing channels clears any downloaded candidate.
...
**Open disk image** is `applyUpdate()`, the only step that is not automatic. It asks macOS to open the verified artifact but does not quit Ensync, restart it, run a silent install, or claim completion. Microsoft Store Windows installations skip this direct updater entirely: the service is `Disabled` for the `store managed` reason and Settings reports **Managed by Store**, and Microsoft Store owns signing, installation, and updates. Development mode, unsupported platforms, invalid or missing HTTPS feed configuration and unsigned installed builds each land in `Disabled` with their own reason; unsigned/unnotarized releases, failed checks and invalid downloads report an errored `Idle` rather than being treated as current.
