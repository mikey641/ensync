...
The signed desktop release workflow performs the production deployment automatically only after macOS attestations, release generation, and Vercel validation pass. Its Windows Store package remains a private Actions artifact pending Partner Center certification, while the direct Windows NSIS installer requires verified signing to enter the public manifest. The same workflow supports a manual macOS-only release that publishes the signed, notarized DMG and records Windows as unavailable (Store-distributed). The workflow requires `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID`; missing credentials fail deployment and leave the previously deployed manifest unchanged.
