...
function liveEvent(response, event) {
  if (response.destroyed || response.writableEnded) return
  if (Number.isSafeInteger(event.revision)) response.write(`id: ${event.revision}\n`)
...
function rawBody(request) {
  return new Promise((resolveBody, reject) => {
    ...
    request.on('data', (chunk) => {
      bytes += chunk.length
      if (bytes > MAX_REQUEST_BYTES) {
        reject(new SyncServiceError('request_too_large', 'The server request is too large.', 413))
        request.destroy()
        return
      }
      chunks.push(chunk)
...
export function createEnsyncSyncServer(options = {}) {
  ...
  const executionOrigin = typeof options.executionOrigin === 'string'
    ? options.executionOrigin.replace(/\/$/, '')
    : null
  const executionToken = typeof options.executionToken === 'string' ? options.executionToken : null
  ...
  function removeLiveSubscriber(username, subscriber) {
    const accountSubscribers = liveSubscribers.get(username)
    if (!accountSubscribers) return
    accountSubscribers.delete(subscriber)
    if (accountSubscribers.size === 0) liveSubscribers.delete(username)
  }

  function closeSessionSubscribers(sessionKeyValue) {
    for (const [username, accountSubscribers] of liveSubscribers) {
      for (const subscriber of accountSubscribers) {
        if (subscriber.sessionKey !== sessionKeyValue) continue
        subscriber.response.end()
        removeLiveSubscriber(username, subscriber)
      }
    }
  }

  function publishWorkspace(username, workspace) {
    const accountSubscribers = liveSubscribers.get(username)
    if (!accountSubscribers) return
    const event = {
    ...
    for (const subscriber of accountSubscribers) liveEvent(subscriber.response, event)
  }

  async function proxyExecution(request, response, url) {
    authenticate(request)
    if (!executionOrigin || !executionToken) {
      throw new SyncServiceError('server_execution_unavailable', 'This Ensync account server has no execution Host.', 503)
    }
    ...
    try {
      ...
      const requestBody = ['GET', 'HEAD'].includes(method) ? undefined : await rawBody(request)
      const upstream = await requestFetch(`${executionOrigin}${url.pathname}${url.search}`, {
        method,
        redirect: 'error',
        ...
        headers: {
          ...
          ...(requestBody && typeof request.headers['content-type'] === 'string'
      ...
      if (buffering) headers['X-Accel-Buffering'] = buffering
    ...
    try {
      if (request.method === 'GET' && url.pathname === '/v1/status') {
        return json(response, 200, {
          service: 'ensync-server',
          version: 2,
          live: true,
          execution: Boolean(executionOrigin && executionToken),
        })
      }
      ...
      if (request.method === 'DELETE' && url.pathname === '/v1/session') {
        ...
        closeSessionSubscribers(session.key)
        return json(response, 200, { authenticated: false })
      }

      if (request.method === 'GET' && url.pathname === '/v1/events') {
        const session = authenticate(request)
        const current = store.read().accounts[session.username]?.workspace
        ...
        response.writeHead(200, liveHeaders())
        const subscriber = { response, sessionKey: session.key }
        const accountSubscribers = liveSubscribers.get(session.username) ?? new Set()
        accountSubscribers.add(subscriber)
        liveSubscribers.set(session.username, accountSubscribers)
        ...
        heartbeat.unref?.()
        const expires = setTimeout(() => response.end(), Math.max(1, session.expiresAt - Date.now()))
        expires.unref?.()
        const cleanup = () => {
          clearInterval(heartbeat)
          clearTimeout(expires)
          removeLiveSubscriber(session.username, subscriber)
        }
      ...
      if (request.method === 'PUT' && url.pathname === '/v1/workspace') {
        ...
        store.write(data)
        publishWorkspace(session.username, account.workspace)
        return json(response, 200, account.workspace)
      }

      if (url.pathname.startsWith('/api/')) {
        return await proxyExecution(request, response, url)
      }
...
if (isEntry) {
  ...
  const { createEnsyncHost } = await import('../host/server.mjs')
  const { AccountSyncService } = await import('../host/account-sync.mjs')
  const executionHost = createEnsyncHost({
    authToken: executionToken,
    accountSyncService: new AccountSyncService({ baseUrl: null, centralExecution: false }),
    chatJobJournalPath: process.env.ENSYNC_SERVER_JOB_JOURNAL_FILE
      ?? resolve('.ensync-server-jobs.json'),
    projectIsolationRoot: process.env.ENSYNC_SERVER_WORKSPACE_ROOT
      ?? resolve('.ensync-server-workspaces'),
    defaultProjectPath: process.env.ENSYNC_SERVER_PROJECT_PATH,
    instanceId: `account-server-${process.pid}`,
  })
  executionHost.listen(0, '127.0.0.1')
  await new Promise((resolveListening, reject) => {
    executionHost.once('listening', resolveListening)
    executionHost.once('error', reject)
  })
  const executionAddress = executionHost.address()
  if (!executionAddress || typeof executionAddress !== 'object') {
    throw new Error('The internal Ensync execution Host did not expose a loopback port.')
  }
  const server = createEnsyncSyncServer({
    store: new FileSyncStore(filePath),
    executionOrigin: `http://127.0.0.1:${executionAddress.port}`,
    executionToken,
  })
  server.once('close', () => executionHost.close())
  server.listen(port, host, () => {
    console.log(`Ensync Server listening on http://${host}:${port}`)
  })
}
